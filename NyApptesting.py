...
def migrate_groups(src_server: TSC.Server, dest_server: TSC.Server, user_map: Dict[str, TSC.UserItem]) -> Dict[str, TSC.GroupItem]:
    """Migrate groups and their memberships from source to destination server using group names."""
    st.subheader("👥 Migrating Groups")
    try:
        src_groups = list(TSC.Pager(src_server.groups))
        dest_groups = list(TSC.Pager(dest_server.groups))
        
        # Create maps by lowercase names
        ...
        # First create all groups that don't exist
        for group_name, src_group in src_group_map.items():
            if group_name not in dest_group_map:
                try:
                    new_group = TSC.GroupItem(src_group.name)
                    created_group = dest_server.groups.create(new_group)
                    dest_group_map[group_name] = created_group
                    st.success(f"✅ Created group: {src_group.name}")
                    migrated_groups += 1
                except Exception as e:
                    st.error(f"❌ Failed to create group {src_group.name}: {str(e)}")
                    skipped_groups += 1
            else:
                st.info(f"ℹ️ Group already exists: {src_group.name}")
                skipped_groups += 1
        
        # Then populate group memberships
        st.subheader("👥➡👤 Migrating Group Memberships")
        for group_name, src_group in src_group_map.items():
            if group_name in dest_group_map:
                dest_group = dest_group_map[group_name]
                ...
                for user in src_group.users:
                    if user.name.lower() in user_map:
                        try:
                            ...
                            st.success(f"✅ Added user {user.name} to group {src_group.name}")
                            added_members += 1
                        except Exception as e:
                            st.error(f"❌ Failed to add user {user.name} to group {src_group.name}: {str(e)}")
                    else:
                        st.warning(f"⚠️ User not found in destination: {user.name}")
                
                st.info(f"ℹ️ Added {added_members} members to group {src_group.name}")
...
def migrate_permissions(
    ...
    """Migrate permissions from source to destination item with proper user/group mapping."""
    try:
        ...
        if item_type == 'workbook':
            ...
            st.warning(f"Unsupported item type: {item_type}")
            return
        ...
        for rule in permissions:
            grantee_type = rule.grantee.tag_name  # "user" or "group"
            capabilities = rule.capabilities
            grantee_id = rule.grantee.id

            if grantee_type == 'user':
                # Match by user ID from the map
                user = next((u for u in user_map.values() if u.id == grantee_id), None)
                if user:
                    grantee = TSC.UserItem.as_reference(user.id)
                else:
                    st.warning(f"⚠️ User with ID '{grantee_id}' not found in destination, skipping permission")
                    continue

            elif grantee_type == 'group':
                if not group_map:
                    st.warning(f"⚠️ Group map is missing. Cannot resolve group ID '{grantee_id}'")
                    continue

                group = next((g for g in group_map.values() if g.id == grantee_id), None)
                if group:
                    grantee = TSC.GroupItem.as_reference(group.id)
                else:
                    st.warning(f"⚠️ Group with ID '{grantee_id}' not found in destination, skipping permission")
                    continue
            else:
                st.warning(f"⚠️ Unsupported grantee type '{grantee_type}', skipping.")
                continue
        ...
        # Apply permissions to the destination item
        if item_type == 'workbook':
...
def migrate_extract_schedules(
    ...
    try:
        ...
        # Filter schedules for the specific item
        item_schedules = []
        for schedule in all_schedules:
            # For workbooks
            if item_type == 'workbook' and hasattr(schedule, 'workbook'):
                if schedule.workbook and schedule.workbook.id == src_item.id:
                    item_schedules.append(schedule)
            # For datasources
            elif item_type == 'datasource' and hasattr(schedule, 'datasource'):
                if schedule.datasource and schedule.datasource.id == src_item.id:
                    item_schedules.append(schedule)
        ...
        # Get existing schedules on destination
        ...
        for schedule in item_schedules:
            try:
                ...
                st.error(f"❌ Failed to process schedule '{schedule.name}': {str(e)}")

        st.success(f"🎉 Successfully migrated {success_count}/{len(item_schedules)} schedules for {item_type} {src_item.name}")
