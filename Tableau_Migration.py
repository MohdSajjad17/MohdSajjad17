...
def get_local_path(type_: str, project_name: str, workbook_name: str) -> str:
    base = os.path.join(os.getcwd(), "tableau_migration", type_, sanitize(project_name))
    os.makedirs(base, exist_ok=True)
    return os.path.join(base, f"{sanitize(workbook_name)}.twbx")
...
def download_workbooks(server, project_id, project_name):
    ...
    for wb in selected:
        ...
        try:
            server.workbooks.download(wb.id, filepath=path)
            files.append((wb, path))
...
def publish_workbooks(server, files_and_wbs, dest_project_id, project_name):
    for wb, path in files_and_wbs:
        ...
        try:
            ...
            # Save to destination folder as record
            dest_path = get_local_path("destination", project_name, wb.name)
...
with st.form("migration_form"):
    ...
    submitted = st.form_submit_button("🚀 Migrate Workbooks")


# Run migration if submitted
if submitted:
    try:
        # Source Login
        ...
        if not files_and_wbs:
            st.warning("No workbooks downloaded.")
            st.stop()
