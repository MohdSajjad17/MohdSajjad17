...
# ------------------------
# Helper: Excel Download Function
# ------------------------
...
# ------------------------
# Export Functions (Updated to use Excel)
# ------------------------
...
def run_import(import_type, uploaded_file, auth):
    ...
    try:
        ...
        # Handle both Excel and CSV imports
...
if mode == "Export Tableau Content":
    ...
    if import_type == "Users":
        uploaded_file = st.file_uploader("📤 Upload Users File (Excel or CSV)", type=["xlsx", "xls", "csv"])
        st.markdown("Upload your user file — columns will be used as-is.")
    else:
        uploaded_file = st.file_uploader("📤 Upload Groups File (Excel or CSV)", type=["xlsx", "xls", "csv"])
        st.markdown("Upload your group file — the first string column value in each row will be used as group name.")
